...
class DispatcherWorkInvoiceByPercentFactory(DjangoModelFactory):
    ...
    start = factory.LazyFunction(lambda: datetime.datetime(2024, 1, 1, 8, 0, tzinfo=UTC))
    end = factory.LazyFunction(lambda: datetime.datetime(2024, 1, 31, 17, 0, tzinfo=UTC))
    percent = 5.0
...
class DispatcherWorkFactory(DjangoModelFactory):
    ...
    dispatcher = factory.SubFactory(UserFactory)
    start = factory.LazyFunction(lambda: datetime.datetime(2024, 1, 15, 9, 0, tzinfo=UTC))
    end = factory.LazyFunction(lambda: datetime.datetime(2024, 1, 15, 17, 0, tzinfo=UTC))
    session = ""
    is_finished = False
    is_paid = False
