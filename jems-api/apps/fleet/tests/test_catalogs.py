...
from apps.fleet.models import (
    CabinType, Card, EngineType, LossPayee, Make, TireSize,
    TransmissionType, TruckMilesReset,
)
...
class TestMake:
    ...
    def test_update_make(self, auth_client):
        ...
        response = client.patch(reverse("make-detail", kwargs={"pk": make.pk}), {"name": "New Name"})
        assert response.status_code == status.HTTP_200_OK
...
class TestCard:
    ...
    def test_create_card(self, auth_client):
        client, _ = auth_client
        response = client.post(reverse("card-list"), {"number": "CARD-XYZ", "is_active": True})
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_toggle_card_active(self, auth_client):
        ...
        response = client.patch(reverse("card-detail", kwargs={"pk": card.pk}), {"is_active": False})
        assert response.status_code == status.HTTP_200_OK
...
class TestTruckMilesReset:
    ...
    def test_delete_reset(self, auth_client):
        client, _ = auth_client
        truck = TruckFactory()
        reset = TruckMilesReset.objects.create(truck=truck, date=datetime.date(2024, 1, 15))
        response = client.delete(reverse("truck-miles-reset-detail", kwargs={"pk": reset.pk}))
