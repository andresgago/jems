...
export default function FinancialReportPage() {
  ...
  function run() {
    ...
    setData(null);
    reportsService
      .financial({ date_begin: start, date_end: end })
  ...
  return (
    <div>
      ...
      {data && (
        <>
          <SectionTable title="Revenues" rows={data.revenues} />
          <SectionTable title="Expenses" rows={data.expenses} />
          <div className="border-top pt-3">
            <table className="table table-sm table-bordered w-auto">
              <tbody>
                <tr>
                  <td className="fw-bold">Total Revenues</td>
                  <td className="text-end">{fmt(data.total_revenues)}</td>
                </tr>
                <tr>
                  <td className="fw-bold">Total Expenses</td>
                  <td className="text-end">{fmt(data.total_expenses)}</td>
                </tr>
