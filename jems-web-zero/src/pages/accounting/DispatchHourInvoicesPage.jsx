...
import { useHourInvoices } from '../../hooks/useHourInvoices';
...
function InvoiceRow({ invoice, onChanged }) {
  ...
  const toggleStatus = async () => {
    ...
    setActioning(true);
    try {
      if (isOpen) await hourInvoicesService.close(invoice.id);
      else await hourInvoicesService.open(invoice.id);
      onChanged();
    } finally {
      setActioning(false);
    }
  ...
  return (
    <tr>
      <td className="align-middle fw-semibold">
        <Link to={`/accounting/invoices/dispatchers-hour/${invoice.id}`} className="text-decoration-none">
          #{invoice.number}
        </Link>
      </td>
      ...
      <td className="align-middle text-center">
        <div className="d-flex gap-1 justify-content-center">
          <Link
            to={`/accounting/invoices/dispatchers-hour/${invoice.id}`}
...
export default function DispatchHourInvoicesPage() {
  const [statusFilter, setStatusFilter] = useState('');
  const [search, setSearch] = useState('');
  const params = statusFilter !== '' ? { status: statusFilter } : {};
  const { items, loading, error, reload } = useHourInvoices(params);
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h5 className="mb-0"><i className="bi bi-clock me-2" />Dispatcher Invoices — By Hour</h5>
        <Link to="/accounting/invoices/dispatchers-hour/create" className="btn btn-sm btn-primary">
